...
void read_simFile(){
  ...
  gSystem->Load("~perezlou/geant4_workdir/ProtonDetectorImplant_build/libProton.sl");
  //TFile *fil=new TFile("simFile_50k.root");
  ...
  //TH2Poly *hpads=pads();
  // TH2Poly *hpads_total=pads("total");
  // TH2Poly *hpads_protons=pads("protons");
  // TH2Poly *hpads_betas=pads("betas");
  // TH2Poly *hpads_all=pads("all");
  hpads->SetMinimum(1);
  ...
  Int_t nbins=hpads->GetNumberOfBins();
  cout<<nbins<<endl;
  ...
  outTree->Branch("Eprotons",Energy_protons,"Epr[nbins]/D");
  ...
  for(Int_t i=0;i<nevents;i++){ //Event Loop	
	...
	for(Int_t j=0;j<nentries;j++){
	  ...
	  //Double_t energy=localTrack->GetEnergyStride();
	  energy_total+=energy;
	  ...
	  //getchar();
	  //cout<<charge<<" Z:"<<z_position<<" X: "<<x_position<<" "<<energy<<"keV "<<realelectrons<<" "<<total_electrons<<endl;
	  ...
	  //hpads->Fill(Z_pos,X_pos,realelectrons);
	...
	for(Int_t bin=1;bin<=nbinsPadPlane;bin++){
	  ...
	  //hEp[bin]->Fill(binContentprotons);
...
TH2Poly *newPads_old(Char_t *hname="hpoly"){
  ...
  pads2->AddBin(-40,-20,-20,20);
  pads2->AddBin(-20,-20,20,20);
  pads2->AddBin(20,-20,40,20);
  return pads2;
}

TH2Poly *newPads(Char_t *hname="hpoly"){
  TH2Poly *pads2=new TH2Poly(hname,"",-150,150,-60,60);
  //pads2->SetName(hname);
  pads2->SetTitle("New  pad plane");
  pads2->AddBin(-90,-30,90,-20);

  pads2->AddBin(-90,-20,-70,20);
  pads2->AddBin(-70,-20,-50,20);
  pads2->AddBin(-50,-20,-30,20);
  pads2->AddBin(-30,-20,-10,20);
  pads2->AddBin(-10,-20,10,20);
  pads2->AddBin(10,-20,30,20);
  pads2->AddBin(30,-20,50,20);
  pads2->AddBin(50,-20,70,20);
  pads2->AddBin(70,-20,90,20);

  pads2->AddBin(-90,20,90,30);
   return pads2;
}
